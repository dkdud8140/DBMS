...
-- TABLE
-- DBMS를 사용하여 데이터를 추가하면 마ㅏ치 EXCEL의 SHEET에 데이터를 저장하는 것처럼 데이터가 저장된다.
-- 실제 DBMS 소프트워에가 저장하는 방식은 고유한 기술이 적용되어있지만
-- DBMS 소프트웨어를 사요ㅕㅇ하는 산용자는 모든 데이터는 표 형식으로 조ㅓ장된 것으로 인식하는 표준형식이다/
--오라클의 데이터 저장 구조 : TableSpace > Tabkle > 데이터들
-- 테이블은 자바 코딩에소 vo클래스를 만드는 원리와 비슷하다.
    --데이터를 취급하는 최소한의 규격, 형식을 지정하여 \생성한다
    
-- DDL 명령중에 Table 생성하기
--  테이블은 데이터를 저장하기 위한 표를 작성하는 것으로
--  각 데이터의 이름을 지정하고 각 데이터의 형식을 지정하여 표를 작성한다.


-- 데이터 타입 
-- 문자열 데이터  
--      캐릭터와 : 저장ㄷ괴느 ㄴ 데이터의 길이가 (규칙으로_)정해진경우,
--        // 데이터 길이가 모두 같은 경우
  --      //고정길이 문자열
        
-- nVARCHAR : 저장되는 데이터의 최대 그리감ㄴ 지정하고 
--  최대 크기보다 짧은 길이의 데이터를 저장할 수 있다
--  쵣개 크기보다 짧은 데이터를 저장하면 자동으로 공간을 축소하여 데이터를 저장한다.
--      가변길이 문자열

-- nVarCHAR ㅣ 한글과 같은 unicode 문자열을 저장할 때 영문 알파벳과 저장하는 방식이 달라서
-- 문제를 일으킬 ㅅ구 있다.
-- nVARCHR 키워드를 사용한다.
...
-- 오라클에서느느 NUBER 키워드로 일괄 지정하여도 상과없다
-- 저장되는 자릿수에[ 따라 자동으로 크기가 조절ㅎ된다.
