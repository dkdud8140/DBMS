...
-- Sub Query 
-- SQL 명령문 내에 또다른 SQL 명령문ㅇ르 포함하는 코드
-- JOIN 대신 사용하는 코드이고
...
때무에 어쩔 수 없는 상황이 아니면 테이블을 ㅂ면경ㅎ라는 것은 별로 좋지 않다
...
-- 학생 점수를 저장할 테이블을 정규화를 하기 위해
-- 재설계를 하였다.
-- 칼럼으로 되어 있던 과목을 데이터화하여 
-- 학번, 과목 형식으로 입력하도록 설계하였다.
-- 이렇게 설계를 하고 나니
-- 학번, 과목명 어떤 키ㅏㄹ람도 단독으로 PK로 설정할 수 없게 되었다.
-- 테이블에 PK가 없으면
-- 데이터의 무결성 유지가 매우 힘든 상황이 될 수 있다.
-- 이 테이블은 학번 + 과목명을 묶어서 PK로 선언을 해야 할 수 밖에 없게 되었다.
-- PK는 단독 칼럼으로 사용하는 것이 가장 좋은데 
-- 단독 칼럼으로 만들 수 없는 경우는 2개 이상의 칼럼을 묶어서 만들어야하는 경우도 있다.
...
삭제나, 갱신을 할 떄 다음과 같은 코드를 사용해야 하는데
코드 사용 중에 실수를 많이 일으킬 수 있다.

삭제나 갱신할 때는 where 조겅문에 PK를 대상으로 
실행하는 것이 좋은데,
아래와 같은 코드를 사용하면
잦은 실수를 반복할 수 있다.

때문에 tbl_score_V2 테이블에는 별로 SEQ 칼럼을 만들고
PK로 설정을 한다.
*/
