...
-- 테이블을 복제한 후 오라클에서는 반드시 PK를 다시 설정해주어야 한다.
-- 테이블을 생성하고 데이터가 있는 상태에서 PK를 변경,추가하는 경우에는
--  PK로 설정하려고 하는 칼럼의 데이터가 PK조건(유일성, NOT BULL)을 
--  만족하지 않는 데이터가 있으면 명령이 실패한다.
-- 대량의 데이터가 저장된 테이블을 변경할 경우에는
--  매우 신중하게 실행을 해야 한다. 
--  또한 미리 데이터 검증을 통하여 제약조건에 위배되는
--  데이터가 있는지 확인을 해야 한다. 
ALTER TABLE tbl_books   -- tbl_books table을 변경
ADD CONSTRAINT PK_ISBN  -- 제약조건을 추가하는데 이름을 PK_ISBN으로
PRIMARY KEY(bk_isbn);   --  bk_isbn 칼럼을 PK로 설정
...
저자항목(칼럼)을 보면 저자가 2명 이상인 데이터가 있고
또한 역자도 2명 이상인 경우가 있다.

데이터를 저장할 칼럼을 크게 설정하여 입력(import)하는 경우에는
문제가 없는데
저자나 역자를 기준으로 데이터를 여러가지 조건을 부여하여
조회를 하려고 하면 문제가 발생할 수 있다.

특히 저자이름으로 GROUPING을 하여 데이터를 조회해보려고 하면
상당히 어려움을 겪을 수 있다.
...
RENAME COLUMN bk_author TO bk_author1 ; -- bk_author를 bk_author1로 변경

-- bk_author2라는 칼럼을 생성
-- 한글가변문자열12로 선언하고 NOT NULL로 설정
...
2. 도서정보와 관련된 저자데이터를 저장할 테이블을 새로 생성한다
...
-- 두 개 이상의 출력된 리스트를 합하여 1개의 리스트로 보여라
...
-- 9791188850280  이 도서에 저자를 추가하고 싶을 때
-- 테이블 의 ba_seq 칼럼에는 이미 등록된 값이 아닌
-- 새로운 숫자를 사용하여 테이터를 추가해야한다
-- 데이터를 추가할 떄 마다 새로운 값이 무언인지 알아햐 하는
-- 새로운 불편할 상황이 생기고 말았슴다~
INSERT INTO tbl_book_author( ba_seq, ba_isbn, ba_author)
VALUES( 35, '9791188850280','홍길동');
INSERT INTO tbl_book_author( ba_seq, ba_isbn, ba_author)
VALUES( 36, '9791188850280','임꺽벙');
INSERT INTO tbl_book_author( ba_seq, ba_isbn, ba_author)
VALUES( 37, '9791188850280','조아영');
