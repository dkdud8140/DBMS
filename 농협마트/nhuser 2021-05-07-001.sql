...
다수의 데이터를 변경하는 명령은 데이터 무결성을 해치는 원인 중의 하나가 된다.

상품테이블을 별도로 분리하고
상품코드, 상품명 형식으로 저장한 후
매입매출 테이블에는 상품명 대신 상품코드를 포함하고
이후에 JOIN을 통해 데이터를 조회하는 것ㅎ이 좋다
...
/*
상품리스트를 추출했는데 매입단가가 0, 또는 매출단가가 0인 경우 매입단가와 매출단가를 계산하기
...
-- JOIN 되ㅏㄴ 상품정보의 상품 이름이 null인 데이터만 조회하기
-- 이 조회에서 데이터가 한 건도 나타나지 않아야 한다.
SELECT IO.io_pname, p.p_name
FROM tbl_iolist IO
    LEFT JOIN tbl_product P
        ON io.io_pname = p.p_name
