...
HAVING 대여일 < '2021-05-03' AND 반납일 IS NULL;
...
-- tbl_score에서 과학과 수학 2과목의 점수만 총점을 계산하고 싶다
SELECT 학생이름, SUM(점수)
FROM tbl_score
...
-- 급여테이블에서 영업직을 제외한 급여 총합계를 계산하고
-- 총 합계가 3000000이상인 데이터만 보여라
SELECT 직업, SUM(급여)
FROM tbl_급여
WHERE 과목 != '영업직'
GROUP BY 직업
...
-- 급여테이블에서 영업직을 제외하고
-- 실 급여가 3000000을 초과하는 데이터만 합산하여 보여라
SELECT 직업, SUM(급여)
FROM tbl_급여
WHERE 과목 != '영업직' AND SUM(급여) > 3000000
GROUP BY 직업 ;

*/


-- 대여일이 2021-05-02 이전이라고 아직 미납된 데이터
SELECT *
FROM view_도서대여정보
WHERE 대여일 < '2021-05-02' AND 반납일 is null ;
...
VALUES(seq_book_rent.NEXTVAL, '2021-04-01','B0015', '9791188850501');

INSERT INTO tbl_book_rent ( br_seq, br_sdate, br_bcode, br_isbn)
VALUES(seq_book_rent.NEXTVAL, '2021-04-01','B0009', '9791188850501');

INSERT INTO tbl_book_rent ( br_seq, br_sdate, br_bcode, br_isbn)
VALUES(seq_book_rent.NEXTVAL, '2021-04-01','B0030', '9791188850501');


-- 대여일이 2021-04-25일 이전이고 아직 미납된 대여정보를 보이고
-- 회원코드 순으로 보여라
SELECT 대여일, 회원코드, 회원명, 회원연락처, 도서명
FROM "VIEW_도서대여정보"
WHERE 대여일 < '2021-05-02' AND 반납일 is null
